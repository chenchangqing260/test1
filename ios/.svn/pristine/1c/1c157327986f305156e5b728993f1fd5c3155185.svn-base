...
- (void)showPickerWithSelectName:(NSString *)provinceName selectArray:(NSMutableArray *)selectArray;{
    _firstAry = [NSArray arrayWithArray:selectArray];
    
    if (provinceName.length > 0) {
        [_firstAry enumerateObjectsUsingBlock:^(NSString *province, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if (_firstAry.count > 0) {
        if (_firstCurrentIndex > 0) {
            if (_firstCurrentIndex - 1 < _firstAry.count) {
                provinceName = _firstAry[_firstCurrentIndex - 1];
            }
        }
    }
