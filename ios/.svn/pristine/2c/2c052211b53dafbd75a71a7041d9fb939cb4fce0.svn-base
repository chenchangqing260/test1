...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _tableTitleFont = _tableBodyTitleFont = [UIFont systemFontOfSize:15];
...
-(void)drawRect:(CGRect)rect{
    ...
    if (_tableTitleString.length>0) {
        ...
        CGSize size = [self sizeOfStringWithMaxSize:CGSizeMake(_tableWidth, _tableChartTitleItemsHeight) textFont:_tableBodyTitleFont.pointSize aimString:_tableTitleString];
        [self drawText:_tableTitleString context:context atPoint:CGRectMake(CGRectGetWidth(self.frame)/2.0 - size.width / 2, _beginSpace + _tableChartTitleItemsHeight/2 - size.height / 2.0, _tableWidth, _tableChartTitleItemsHeight) WithColor:_tableTitleColor font:_tableBodyTitleFont];
    ...
    if (_colTitleArr.count>0) {
        ...
        for (NSInteger i = 0; i<_colTitleArr.count; i++) {
            ...
            CGSize size = [self sizeOfStringWithMaxSize:CGSizeMake(wid, _minHeightItems) textFont:_tableTitleFont.pointSize aimString:_colTitleArr[i]];
            ...
            if (i==0) {
                ...
                if (firArr.count>=2) {
                    ...
                    [self drawText:firArr[0] context:context atPoint:CGRectMake(lastX + wid / 2.0 + wid / 4.0 - size.width / 2, _lastY + _minHeightItems / 4.0 -size.height / 2.0, wid, _minHeightItems / 2.0) WithColor:_bodyTextColor font:_tableBodyTitleFont];
                    size = [self sizeOfStringWithMaxSize:CGSizeMake(wid, _minHeightItems) textFont:14 aimString:firArr[1]];

                    [self drawText:firArr[1] context:context atPoint:CGRectMake(lastX + wid / 4.0 - size.width / 2.0, _lastY + _minHeightItems / 2.0 + _minHeightItems / 4.0 - size.height / 2.0, size.width+5, _minHeightItems / 2.0) WithColor:_bodyTextColor font:_tableBodyTitleFont];
                }else{
        ...
        for (NSInteger j = 0; j< model.dataArr.count; j++) {
            ...
            if ([rowItems isKindOfClass:[NSArray class]]) {
                ...
                for (NSInteger n = 0; n<[rowItems count]; n++) {
                    ...
                    CGSize size = [self sizeOfStringWithMaxSize:CGSizeMake(wid, perItemsHeightByMaxCount) textFont:_tableBodyTitleFont.pointSize aimString:rowItems[n]];
//                    P_M(lastX + wid / 2 - size.width / 2.0, _lastY + (n+1) * perItemsHeightByMaxCount - perItemsHeightByMaxCount / 2.0 - size.height / 2.0)
                    [self drawText:rowItems[n] context:context atPoint:CGRectMake(lastX + wid / 2 - size.width / 2.0, _lastY + (n+1) * perItemsHeightByMaxCount - perItemsHeightByMaxCount / 2.0 - size.height / 2.0, size.width, size.height) WithColor:_bodyTextColor font:_tableBodyTitleFont];
                }
                
            }else{
                
                CGSize size = [self sizeOfStringWithMaxSize:CGSizeMake(wid, model.maxCount * _minHeightItems) textFont:_tableBodyTitleFont.pointSize aimString:rowItems];

                  [self drawText:rowItems context:context atPoint:CGRectMake(lastX + wid / 2 - size.width / 2.0,  _lastY + model.maxCount * _minHeightItems - model.maxCount * _minHeightItems / 2.0 - size.height / 2.0, size.width, size.height) WithColor:_bodyTextColor font:_tableBodyTitleFont];
            }
            lastX += wid;
