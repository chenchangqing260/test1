...
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _waveCycle=1.0;
        increase=NO;
        _waterColor=[UIColor colorWithHex:0x33cfda alpha:0.05];
        _waterLineY=frame.size.height - 70;
...
//    [attriButedText drawAtPoint:textPoint];
    ...
    for(float x=0;x<=kSCREEN_WIDTH;x++){
    ...
    //后波浪绘制
    CGContextSetFillColorWithColor(context, [[UIColor colorWithHex:0x33cfda alpha:0.1] CGColor]);
    CGPathAddLineToPoint(backPath, nil, kSCREEN_WIDTH, rect.size.height);
    ...
    //后波浪反色绘制
    CGPathAddLineToPoint(backReversePath, nil, kSCREEN_WIDTH, rect.size.height);
...
//    [attriButedText drawAtPoint:textPoint];
    ...
    CGPathAddLineToPoint(frontPath, nil, kSCREEN_WIDTH, rect.size.height);
    ...
    //前波浪反色绘制
    CGPathAddLineToPoint(frontReversePath, nil, kSCREEN_WIDTH, rect.size.height);
