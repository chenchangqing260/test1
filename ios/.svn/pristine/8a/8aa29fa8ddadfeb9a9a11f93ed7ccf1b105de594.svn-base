...
@interface YJShowAreaoverController ()

@property (nonatomic, strong) UIViewController *contentViewController;
@property (nonatomic, assign) CGSize popoverContentSize;    //弹出框size
@property (nonatomic, strong) YJShowoverView *popoverView;   //背景视图，有箭头

@property (nonatomic, assign) CGRect contentViewRect;       //内容视图大少

@end

@implementation YJShowAreaoverController
...
- (void)viewDidLoad {
    ...
    self.contentViewController.view.layer.cornerRadius = 5;
...
- (void)presentPopoverFromRect:(CGRect)fromRect inView:(UIView *)inView permitterArrowDirections:(YJPopoverArrowDirection)permitterArrowDirection animated:(BOOL)animated
{
    ...
    switch (permitterArrowDirection) {
        ...
        {
            self.contentViewRect = CGRectMake(0, 8, self.popoverContentSize.width, self.popoverContentSize.height);
            ...
            if (CGRectGetMidX(contentRect) <= CGRectGetMidX(kYJCurrentWindow.bounds)) {
                ...
                //箭头向右偏移15
                self.popoverView.arrowLocation = CGRectGetWidth(self.popoverView.bounds) - (viewRight - CGRectGetMidX(contentRect))+15;
                //向左偏移15
                self.view.frame = CGRectMake(viewRight - self.popoverContentSize.width-15, CGRectGetMaxY(contentRect) + kPopoverViewMaginToBtn, self.popoverContentSize.width, self.popoverContentSize.height);
            }
        }
            break;
...
#pragma mark -- setter
- (YJShowoverView *)popoverView
{
    if (_popoverView == nil) {
        _popoverView = [[YJShowoverView alloc] initWithFrame:CGRectMake(0, 0, self.popoverContentSize.width, self.popoverContentSize.height)];
    }
    return _popoverView;
}
