...
@property (nonatomic) UIView *sliderPanView;
@property (nonatomic) float panStartSliderValue;
@property (nonatomic) CGFloat panStartX;
@property (nonatomic) BOOL playBeforePan;
...
//    }
        if ([[urlString substringWithRange:NSMakeRange(0, 4)] isEqualToString:@"http"]) {
            ...
            DLog(@"%@", fileURL);
            AVAsset *movieAsset  = [[AVURLAsset alloc]initWithURL:fileURL options:nil];
            AVPlayerItem *playerItem = [AVPlayerItem playerItemWithAsset:movieAsset];
            return playerItem;
        }
}
...
//        [self.lightSlider addTarget:self action:@selector(updateLightValue:) forControlEvents:UIControlEventValueChanged];
        ...
        [self.bottomView addSubview:panView];
        
        [panView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.progressSlider).with.offset(0);
            make.right.equalTo(self.progressSlider).with.offset(0);
            make.top.equalTo(self.bottomView).with.offset(0);
            make.bottom.equalTo(self.bottomView).with.offset(0);
        }];
        
        UIPanGestureRecognizer *panGesture = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(didProgressPanGestureTriggered:)];
        [panView addGestureRecognizer:panGesture];
        self.sliderPanView = panView;
...
- (void)didProgressPanGestureTriggered:(UIPanGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        self.playBeforePan = self.player.rate == 1;
        [self.player pause];
        self.panStartX = [gesture locationInView:gesture.view].x;
        self.panStartSliderValue = self.progressSlider.value;
    } else if(gesture.state == UIGestureRecognizerStateChanged) {
        CGFloat x = [gesture locationInView:gesture.view].x;
        CGFloat delta = x - self.panStartX;
        CGFloat progressDelta = delta / self.sliderPanView.width * (self.progressSlider.maximumValue - self.progressSlider.minimumValue);
        self.progressSlider.value = self.panStartSliderValue + progressDelta;
    } else if(gesture.state == UIGestureRecognizerStateEnded) {
        [self updateProgress:self.progressSlider];
        if (self.playBeforePan) {
            [self.player play];
        }
    } else {
        if (self.playBeforePan) {
