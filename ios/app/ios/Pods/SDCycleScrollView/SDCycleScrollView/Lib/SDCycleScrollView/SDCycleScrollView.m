...
- (void)createIndexLabel
{
    if (nil == self.label) {
        ...
        if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
            ...
            size = CGSizeMake(self.imagePathsGroup.count * self.pageControlDotSize.width * 1.2+30, self.pageControlDotSize.height);
        }
        ...
        if (self.pageControlAliment == SDCycleScrollViewPageContolAlimentTop) {
            ...
            y = 10;
        }else if(self.pageControlAliment == SDCycleScrollViewPageContolAlimentRightTop){
            if([UIScreen mainScreen].bounds.size.height==736.0f){
                x = self.mainView.sd_width - size.width +25;
                y = self.mainView.sd_height - size.height +25;
            }else{
                x = self.mainView.sd_width - size.width - 20;
                y = self.mainView.sd_height - size.height -30;
            }
        }else{
            x = (self.sd_width - size.width) * 0.5;
        ...
        self.label = [[UILabel alloc] initWithFrame:pageControlFrame];
//        self.label.backgroundColor = [UIColor colorWithWhite:0 alpha:0.5];
        self.label.text = [NSString stringWithFormat:@"1/%ld", 1];
...
- (void)setImageURLStringsGroup:(NSArray *)imageURLStringsGroup
{
    ...
    self.pageNum = self.imagePathsGroup.count;
}
...
- (void)setLocalizationImageNamesGroup:(NSArray *)localizationImageNamesGroup
{
    ...
    self.pageNum = localizationImageNamesGroup.count;
}
...
- (void)setupPageControl
{
    ...
    switch (self.pageControlStyle) {
        case SDCycleScrollViewPageContolStyleAnimated:
        {
            ...
            pageControl.dotSize = CGSizeMake(20,7);
            ...
            pageControl.dotImage = [UIImage imageNamed:@"icon_banner_normal"];
            pageControl.currentDotImage = [UIImage imageNamed:@"icon_banner_selected"];
...
- (void)layoutSubviews
{
    ...
    if (self.pageControlAliment == SDCycleScrollViewPageContolAlimentTop) {
        ...
        y = 10;
         ...
         x = self.mainView.sd_width - size.width - 10;
        y = self.mainView.sd_height - size.height - 35;
    }else{
        x = (self.sd_width - size.width) * 0.5;
        if (self.pageControlAliment == SDCycleScrollViewPageContolAlimentRight) {
            x = self.mainView.sd_width - size.width - 10;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
        ...
        if (self.label) {
            self.label.text = [NSString stringWithFormat:@"%ld/%ld", pageControl.currentPage + 1, self.pageNum];
        }
    } else {
        UIPageControl *pageControl = (UIPageControl *)_pageControl;
        pageControl.currentPage = indexOnPageControl;
