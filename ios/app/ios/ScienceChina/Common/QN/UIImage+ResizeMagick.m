...
{
    UIImage *newImage = [self  resizedImageByMagick: [NSString stringWithFormat:@"%f%@%f", size.width, @"x", size.height]];
    
    NSData *imgData1 = UIImageJPEGRepresentation(newImage, quality);
...
{
    UIImage *newImage = [self  resizedImageByMagick:@"540x119"];
...
- (NSData *) resizedImageWithMaximumSize: (CGSize) size compressionQuality:(CGFloat)quality{
    ...
    CGFloat scale_ratio = width_ratio < height_ratio ? width_ratio : height_ratio;
    CGImageRelease(imgRef);
    UIImage* anOtherImg = [self drawImageInBounds: CGRectMake(0, 0, round(original_width * scale_ratio), round(original_height * scale_ratio))] ;
    return UIImageJPEGRepresentation(anOtherImg, quality);
}
