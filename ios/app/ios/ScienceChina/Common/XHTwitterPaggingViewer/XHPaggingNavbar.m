...
- (void)reloadData {
    ...
    [self.titles enumerateObjectsUsingBlock:^(NSString *title, NSUInteger idx, BOOL *stop) {
        CGFloat x = 0;
        if (deviceRatio) {
            x = idx * (deviceWidth / deviceRatio);
        }
        CGRect titleLabelFrame = CGRectMake(x, 8, CGRectGetWidth(self.bounds), 20);
...
- (SMPageControl *)pageControl {
    if (!_pageControl) {
        ...
        _pageControl.frame = (CGRect){0, 25, CGRectGetWidth(self.bounds), 25};
