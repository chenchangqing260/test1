...
#pragma mark - 注册通知
-(void)registerNotice{
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(loadMyChannelsData) name:kNOTOICE_REFRESHINDEXVIEW_INFO object:nil];
}
...
-(void)setupLoadingView{
    ...
    NSInteger loadImage_num=0;
    NSInteger loadLabelnum=0;
    ...
    _loadImageView.frame = CGRectMake((MAIN_SCREEN_WIDTH-loadW)/2.0, (self.view.frame.size.height-loadH)/2.0-loadImage_num, loadW, loadH);
    //_loadImageView.hidden = YES;
     _loadImageView.image = [UIImage imageNamed:@"zhanweitu"];
    [self.view addSubview:_loadImageView];
    [self.view insertSubview:_loadImageView atIndex:0];
    
    _loadLabel = [[UILabel alloc] init];
    _loadLabel.frame = CGRectMake((MAIN_SCREEN_WIDTH-loadW)/2.0-20, (self.view.frame.size.height-loadH)/2.0-loadLabelnum, 220, loadH);
...
-(void)refreshChannelView{
    ...
    [[AppDelegate appdelegate] closeNotiNetwork];
}
#pragma mark - 网络数据加载
//获取分类列表
-(void)loadData{
