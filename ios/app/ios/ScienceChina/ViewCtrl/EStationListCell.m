...
@implementation EStationListCell
...
+(NSString*)ID{
    return @"EStationListCell";
}

+(EStationListCell *)newCell{
    return [[NSBundle mainBundle]loadNibNamed:@"EStationListCell" owner:nil options:nil][0];
    ...
    if ([_station.si_is_concern intValue] == Attented) {
        ...
        [[WebAccessManager sharedInstance]delStationWithSiId:_station.si_id Completion:^(WebResponse *response, NSError *error) {
            if (response.success) {
                ...
                // 刷新TableView
                [self.delegate refreshTableViewCellWithIndexPath:_indexPath];
            }
        }];
    }else{
        // 未关注的，进行关注操作
